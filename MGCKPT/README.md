...
| [P6_TP.md](P6_TP.md) | P6 vLLM 并行部署（PP 交付 / TP 记录）| **PP 已交付（2026-08-17）** — 真实 64 层 QwQ-32B 跑 PP=4（16 层/卡）：我们的 W4A8 对 BF16 **2.08×**、对我们自己的 W8A8 **1.11×**（batch=1），**零 kernel 改动**，优势随 PP 不减反增。但 **PP 给容量不给延迟**（D10）。TP 的全部结论留在本文件备查，跟进转 `P6.5_TP.md` |
